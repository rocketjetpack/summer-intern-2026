...
- **Explain why PAM4 links require Forward Error Correction, and what FEC costs in return.**
  ...
  The cost is latency: encoding and decoding both take time, on the order of 100-200ns depending on which FEC variant is running, plus a small chunk of the link's raw bit rate spent on the redundant symbols. For most Ethernet traffic that's nothing. It matters a lot more once you get to RDMA/InfiniBand, a stretch topic later in the program where the whole latency budget for an operation is measured in single-digit microseconds, and a few hundred nanoseconds stops being a rounding error.
...
- **Compare multimode and single-mode fiber as a distance/cost trade-off.**
  ...
  Multimode fiber (OM3, OM4, OM5) uses cheaper VCSEL-based optics matched to its wider core, and stays cheaper for short runs: in a rack, between rows. Single-mode (OS2) uses costlier laser-based optics and is the on-ramp to long-haul DWDM transport (OTN, a stretch topic later in the program).
...
- **Compare parallel optics and WDM-multiplexed optics as two answers to "how do I get N lanes across a fiber plant."**
  ...
  It's the same tension Week 4 raised with ECMP vs. LAG, just at the physical layer: parallel optics is cheap and simple but eats into fiber-plant capacity as bandwidth grows (a real constraint when a spine-leaf fabric only has so many fiber runs between rows), while WDM optics costs more per port but decouples bandwidth from fiber count. OTN and long-haul DWDM transport (a stretch topic later in the program) extend this same idea, just with dozens of wavelengths over much longer distances instead of 4 wavelengths over a couple of kilometers.
...
## The OTN Bridge (Stretch Topic)

OTN and long-haul DWDM transport pick up right where this week's WDM discussion leaves off, though it isn't on the current week-by-week schedule. This week's CWDM4/LR4 optics multiplex a handful of wavelengths over a couple of kilometers, inside or between buildings on the same site. OTN/DWDM transport multiplexes dozens of wavelengths over tens to thousands of kilometers between sites. The link power budget you compute in this week's code lab is the same calculation an OTN/DWDM designer runs, just with amplifiers, dispersion compensation, and a lot more decibels to account for.

*Forward reference: the FEC and latency discussion above is also a direct on-ramp to InfiniBand & RDMA, another stretch topic, where link-layer latency additions that are negligible for ordinary Ethernet traffic become a meaningful fraction of a microsecond-scale RDMA operation.*

## The Week 7 Bridge

Week 7 turns back up the stack from the physical layer to the network layer, and replaces an assumption every week so far has quietly carried: that addresses are IPv4. Week 7 builds fluency in IPv6, the address family the program's final production deployment actually runs on: reading addresses, scopes and prefixes, and the link-local layer, with Neighbor Discovery observed live in the lab. The routing story (letting routers discover paths themselves, and then BGP) belongs to weeks 8 through 10, and it is told in IPv6 from the start; that is exactly why the fluency comes first.
